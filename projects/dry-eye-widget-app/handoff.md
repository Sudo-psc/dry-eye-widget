...
1. Run the app locally and manually inspect left/right docking, hover amplification, click-to-undock, opacity, and size changes.
2. Inspect the same docking behavior on Windows before packaging because off-screen window positioning can differ by platform.
3. Watch real-use CPU/battery behavior now that the dynamic orb effect is enabled by default.
